...
class MainActivity : FlutterActivity() {
    ...
    private fun launchRussianVoiceInput() = try {
        NativeVoiceCaptureActivity.intent(
            this,
    ...
    private fun openWifiSettings() = try {
        startActivity(Intent(SettingsIntentPolicy.wifiAction()))
        true
    } catch (e: Exception) {
        false
    }
    ...
    @Suppress("DEPRECATION")
    private fun getSystemStatus(): Map<String, Serializable> {
        val wifi = applicationContext.getSystemService(Context.WIFI_SERVICE) as WifiManager
        val info = wifi.connectionInfo
        val connected = info != null && info.networkId != -1
        val rawSsid = if (connected) info.ssid ?: "" else ""
        val ssid = rawSsid.removePrefix("\"").removeSuffix("\"").takeUnless { it == "<unknown ssid>" } ?: ""
        val level = if (connected) WifiManager.calculateSignalLevel(info.rssi, 5) else 0
        val battery = registerReceiver(null, IntentFilter(Intent.ACTION_BATTERY_CHANGED))
        val plugged = battery?.getIntExtra(BatteryManager.EXTRA_PLUGGED, 0) ?: 0
        val batteryStatus = BatteryStatus.select(
            mcuCapacity = readTextOrNull("/sys/class/power_supply/mcu_slave_battery/capacity"),
            mcuStatus = readTextOrNull("/sys/class/power_supply/mcu_slave_battery/status"),
            broadcastLevel = battery
                ?.getIntExtra(BatteryManager.EXTRA_LEVEL, -1)
                ?.takeIf { it >= 0 },
            broadcastScale = battery
                ?.getIntExtra(BatteryManager.EXTRA_SCALE, -1)
                ?.takeIf { it > 0 },
            plugged = plugged != 0,
        )
        return mutableMapOf<String, Serializable>(
            "wifiConnected" to connected,
            "wifiSsid" to ssid,
            "wifiLevel" to level,
            "pluggedIn" to batteryStatus.charging,
        ).apply {
            batteryStatus.percent?.let { put("batteryPercent", it) }
        }
    }
