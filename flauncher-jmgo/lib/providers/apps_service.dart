...
class AppsService extends ChangeNotifier {
  ...
  List<CategoryWithApps> get categoriesWithApps => _categoriesWithApps
      .map((item) => CategoryWithApps(
          item.category, UnmodifiableListView(item.applications)))
  ...
  Future<void> _init() async {
    ...
    if (!_database.wasCreated && _categoriesWithApps.isNotEmpty) {
      _applications = await _database.listApplications();
      if (await _assignUncategorizedApps()) {
        _categoriesWithApps = await _database.listCategoriesWithVisibleApps();
      }
      _listenForAppChanges();
      _initialized = true;
      notifyListeners();
      unawaited(_refreshStateInBackground());
      return;
    }
  ...
  Future<void> _refreshStateInBackground() async {
    try {
      await _refreshState();
    } catch (error, stackTrace) {
      debugPrint("Could not refresh applications in background: $error");
      debugPrintStack(stackTrace: stackTrace);
    }
  }

  void _listenForAppChanges() {
    _fLauncherChannel.addAppsChangedListener((event) async {
      switch (event["action"]) {
        case "PACKAGE_ADDED":
        case "PACKAGE_CHANGED":
          await _database
              .persistApps([_buildAppCompanion(event["activitiyInfo"])]);
          break;
        case "PACKAGES_AVAILABLE":
          await _database.persistApps((event["activitiesInfo"] as List<dynamic>)
              .map(_buildAppCompanion)
              .toList());
          break;
  ...
  AppsCompanion _buildAppCompanion(dynamic data) => AppsCompanion(
        ...
        version: Value(data["version"] ?? "(неизвестно)"),
  ...
  Future<void> _initDefaultCategories() => _database.transaction(() async {
        final tvApplications =
            _applications.where((element) => element.sideloaded == false);
        final nonTvApplications =
            _applications.where((element) => element.sideloaded == true);
        if (tvApplications.isNotEmpty) {
          await addCategory("Приложения для ТВ", shouldNotifyListeners: false);
          final tvAppsCategory = _categoriesWithApps
              .map((e) => e.category)
              .firstWhere((element) => element.name == "Приложения для ТВ");
        ...
        if (nonTvApplications.isNotEmpty) {
          ...
          final nonTvAppsCategory = _categoriesWithApps
              .map((e) => e.category)
              .firstWhere((element) => element.name == "Остальные приложения");
  ...
  Future<void> _refreshState({bool shouldNotifyListeners = true}) async {
    await _database.transaction(() async {
      ...
      final appsRemovedFromSystem = (await _database.listApplications())
          .where((app) => !appsFromSystem.any(
              (systemApp) => systemApp.packageName.value == app.packageName))
  ...
  Future<bool> _assignUncategorizedApps() async {
    final assignedPackages = _categoriesWithApps
    ...
    for (final app in _applications
        .where((app) => !assignedPackages.contains(app.packageName))) {
      final defaultCategoryName =
          app.sideloaded ? "Остальные приложения" : "Приложения для ТВ";
      final matchingCategories = _categoriesWithApps
          .where((entry) => entry.category.name == defaultCategoryName);
      if (matchingCategories.isEmpty) continue;

      final category = matchingCategories.first.category;
      final order = await _database.nextAppCategoryOrder(category.id) ?? 0;
      ...
      assignedPackages.add(app.packageName);
      changed = true;
    }
    return changed;
  }
  ...
  Future<void> saveOrderInCategory(Category category) async {
    final applications = _categoriesWithApps
        .firstWhere((element) => element.category.id == category.id)
        .applications;
  ...
  Future<void> addCategory(String categoryName,
    ...
    for (int i = 0; i < _categoriesWithApps.length; ++i) {
      ...
      orderedCategories.add(
          CategoriesCompanion(id: Value(category.id), order: Value(i + 1)));
    }
  ...
  Future<void> moveCategory(int oldIndex, int newIndex) async {
    ...
    for (int i = 0; i < _categoriesWithApps.length; ++i) {
      final category = _categoriesWithApps[i].category;
      orderedCategories
          .add(CategoriesCompanion(id: Value(category.id), order: Value(i)));
    }
    await _database.updateCategories(orderedCategories);
    _categoriesWithApps = await _database.listCategoriesWithVisibleApps();
    notifyListeners();
