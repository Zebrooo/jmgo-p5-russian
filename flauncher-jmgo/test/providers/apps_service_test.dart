...
void main() {
  group("AppsService initialised correctly", () {
    test("shows cached apps immediately and refreshes them in background", () async {
      final channel = MockFLauncherChannel();
      final database = MockFLauncherDatabase();
      final app = fakeApp(packageName: "org.smarttube.stable");
      final category = fakeCategory(name: "Приложения для ТВ");
      ...
      when(database.listApplications()).thenAnswer((_) => Future.value([app]));
      ...
      expect(service.categoriesWithApps.single.applications.single, app);
      await pumpEventQueue();
      verify(channel.getApplications()).called(1);
    });

    test("adds an uncategorized TV app to the TV category", () async {
      final channel = MockFLauncherChannel();
      final database = MockFLauncherDatabase();
      final app = fakeApp(
      ...
      when(channel.getApplications()).thenAnswer((_) => Future.value([]));
      when(channel.applicationExists(app.packageName))
          .thenAnswer((_) => Future.value(true));
      when(database.listApplications()).thenAnswer((_) => Future.value([app]));
      when(database.listCategoriesWithVisibleApps())
          .thenAnswer((_) => Future.value([CategoryWithApps(category, [])]));
      ...
      await untilCalled(database.insertAppsCategories(any));
    ...
    test("with empty database", () async {
      ...
      final tvApplicationsCategory = fakeCategory(name: "Приложения для ТВ");
      final nonTvApplicationsCategory =
          fakeCategory(name: "Остальные приложения");
      when(database.listCategoriesWithVisibleApps())
          .thenAnswer((_) => Future.value([
      ...
      verifyInOrder([
        ...
        database.insertCategory(
          CategoriesCompanion.insert(name: "Приложения для ТВ", order: 0),
        ),
        ...
        database.insertCategory(
          CategoriesCompanion.insert(name: "Остальные приложения", order: 0),
        ),
        database.insertAppsCategories([
          AppsCategoriesCompanion.insert(
  ...
  test("reorderApplication changes application order in-memory", () async {
    ...
    expect(
        appsService.categoriesWithApps[0].applications[0].packageName, "app.2");
    expect(
        appsService.categoriesWithApps[0].applications[1].packageName, "app.1");
  });
  ...
  test("hideApplication hides application", () async {
    ...
    when(database.listApplications())
